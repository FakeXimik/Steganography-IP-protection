...
# ==========================================
# GLOBAL FEC INITIALIZATION (Prevents CPU Bottleneck)
# ==========================================
...
def generate_real_payloads(batch_size, device):
    ...
    Converts real UUIDs + Reed-Solomon parity into a bit tensor.
    """
    all_bits = []
    for _ in range(batch_size):
        # 1. Generate random 16 bytes (UUID)
        raw_uuid_bytes = uuid.uuid4().bytes
        
        # 2. Encode with Reed-Solomon (using the internal rs object for speed)
        # This results in 26 bytes (16 data + 10 parity)
        encoded_bytes = FEC_PIPELINE.rs.encode(raw_uuid_bytes)
        
        # 3. Fast Bit Conversion (Big Endian)
        bits = []
...
# ==========================================
# THE MASTER TRAINING LOOP (With Noise Ramping)
# ==========================================
def run_training_loop():
    ...
    # Start with a low image penalty (0.1) so it prioritizes the message first
    criterion = HiDDeNLoss(lambda_i=0.1, lambda_g=0.001).to(device)
    ...
    for epoch in range(epochs):
        ...
        for i, (images, _) in enumerate(train_loader):
            ...
            # Keep gradient clipping just to be safe
